...
pipe = CogVideoXImagesToVideoPipeline.from_pretrained("/media/vahid/DATA/projects/cogvideox-factory/models/CogVideoX-5b-I2V", torch_dtype=torch.bfloat16).to("cuda")
# pipe = CogVideoXVideoToVideoPipeline.from_pretrained("THUDM/CogVideoX-5b-I2V", torch_dtype=torch.bfloat16).to("cuda")
lora_path = "/media/vahid/DATA/projects/cogvideox-factory/runs/cogvideox-lora__optimizer_adamw__steps_2000__lr-schedule_cosine_with_restarts__learning-rate_1e-3/checkpoint-2000"
lora_rank = 256
lora_alpha = 128
...
export_to_video(video_inp[:30], f"input_gengs_video_{traj}.mp4", fps=8)
video = pipe(images=video_inp[:num_frames], prompt=prompt, negative_prompt=negative_prompt,
                          use_dynamic_cfg=True, num_inference_steps=50)
export_to_video(video.frames[0][:30], f"output_vid_gengs_{traj}_g5_50_6_lora.mp4", fps=8)

export_to_video(video_inp_lora[:30], f"input_lora_video_{traj}.mp4", fps=8)
video = pipe(images=video_inp_lora[:num_frames], prompt=prompt, negative_prompt=negative_prompt,
                          use_dynamic_cfg=True, num_inference_steps=50)
export_to_video(video.frames[0][:30], f"output_vid_lora_{traj}_g5_50_6_lora.mp4", fps=8)