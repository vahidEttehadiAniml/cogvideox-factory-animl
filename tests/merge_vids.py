...
from moviepy.video.fx import all as vfx  # Import the `all` module for effects
...
def merge_videos_with_reversed(input_videos, output_path):
    """
    ...
    for i, video_path in enumerate(input_videos):
        ...
        # Reverse the video using clip.fx()
        reversed_clip = clip.fx(vfx.time_mirror)

        # Gather the remaining videos
        other_clips = [VideoFileClip(v) for j, v in enumerate(input_videos) if j != i]

        # Combine reversed clip with the rest of the clips
        for ii, clip in enumerate(other_clips):

            final_video = concatenate_videoclips([reversed_clip] + [clip])
            merged_vid_path = f'{output_path}/{i}_{ii}.mp4'
            final_video.write_videofile(merged_vid_path, codec="libx264")
