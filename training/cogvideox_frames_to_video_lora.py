...
pipe = CogVideoXFramesToVideoPipeline.from_pretrained("THUDM/CogVideoX-5b-I2V", torch_dtype=torch.bfloat16).to("cuda")
...
def run_validation(
    ...
    pipe = CogVideoXFramesToVideoPipeline.from_pretrained(
...
def main(args):
    ...
    def save_model_hook(models, weights, output_dir):
        if accelerator.is_main_process:
            ...
            CogVideoXFramesToVideoPipeline.save_lora_weights(
                output_dir,
    ...
    def load_model_hook(models, input_dir):
        ...
        lora_state_dict = CogVideoXFramesToVideoPipeline.lora_state_dict(input_dir)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if random.random() < args.condition_frames_dropout:
                    drop_latent = torch.zeros_like(image_latents[:,1:-1])
                    mid_ind = drop_latent.shape[1]//2
                    drop_latent[:, mid_ind] = image_latents[:, mid_ind] * 1.0
                    image_latents[:,1:-1] = drop_latent
